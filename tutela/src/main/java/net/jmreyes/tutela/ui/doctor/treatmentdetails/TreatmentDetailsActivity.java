...
public class TreatmentDetailsActivity extends BaseActivity implements TreatmentDetailsView {
    ...
    private HashMap<String, ArrayList<HistoryItem>> medicationHistory;
    private HashMap<String, ArrayList<HistoryItem>> symptomsHistory;
    ...
    private void populateViewDetails() {
        ...
        for (Map.Entry<String, ArrayList<HistoryItem>> entry : medicationHistory.entrySet()) {
            ...
            ArrayList<HistoryItem> datesAndAnswers = entry.getValue();
            for (int i = 0; i < datesAndAnswers.size(); i++) {
                ...
                date.setText(datesAndAnswers.get(i).getDate());
                answer.setText(datesAndAnswers.get(i).getAnswer());
                view.addView(row);

                PointValue p = new PointValue(datesAndAnswers.get(i).getDateLong(), datesAndAnswers.get(i).getIndex());
                p.setLabel(datesAndAnswers.get(i).getDate().toCharArray());
                values.add(p);
            }

            List<AxisValue> yAxisValues = new ArrayList<AxisValue>();
            yAxisValues.add(new AxisValue(0, "NO".toCharArray()));
            yAxisValues.add(new AxisValue(1, "YES".toCharArray()));

            Line line = new Line(values).setColor(getResources().getColor(R.color.colorAccent)).setCubic(false);
            line.setHasLabelsOnlyForSelected(true);
            List<Line> lines = new ArrayList<Line>();
            lines.add(line);

            Axis y = new Axis();
            y.setTextColor(getResources().getColor(R.color.text_main));
            y.setAutoGenerated(true);
            y.setValues(yAxisValues);
        ...
        for (Map.Entry<String, ArrayList<HistoryItem>> entry : symptomsHistory.entrySet()) {
            ...
            LineChartView chart = (LineChartView) view.findViewById(R.id.chart);
            List<PointValue> values = new ArrayList<PointValue>();
            HashMap<Integer, AxisValue> yAxisValues = new HashMap<Integer, AxisValue>();
            ...
            for (int i = 0; i < datesAndAnswers.size(); i++) {
                ...
                if (!yAxisValues.containsKey(datesAndAnswers.get(i).getIndex())) {
                    yAxisValues.put(datesAndAnswers.get(i).getIndex(), new AxisValue(datesAndAnswers.get(i).getIndex(), datesAndAnswers.get(i).getAnswer().toCharArray()));
                }
            }
            ...
            y.setTextColor(getResources().getColor(R.color.text_main));
            y.setAutoGenerated(true);
            y.setValues(new ArrayList<AxisValue>(yAxisValues.values()));
