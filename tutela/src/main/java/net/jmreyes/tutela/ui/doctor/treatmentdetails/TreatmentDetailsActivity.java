...
public class TreatmentDetailsActivity extends BaseActivity implements TreatmentDetailsView {
    ...
    private void populateViewDetails() {
        ...
        patientName.setText(treatment.getPatientName() + ": Check-Ins");
        ...
        for (Map.Entry<String, ArrayList<HistoryItem>> entry : medicationHistory.entrySet()) {
            ...
            for (int i = 0; i < datesAndAnswers.size(); i++) {
                ...
                HistoryItem item = datesAndAnswers.get(i);
                String itemDate = item.getDate();
                String itemAnswer = item.getAnswer();
                int itemIndex = item.getIndex();
                long itemDateLong = item.getDateLong();

                date.setText(itemDate);
                answer.setText(itemAnswer);
                view.addView(row);

                PointValue p = new PointValue(itemDateLong, itemIndex);
                p.setLabel(itemDate.toCharArray());
                values.add(p);
            }

            List<AxisValue> yAxisValues = new ArrayList<AxisValue>();
            yAxisValues.add(new AxisValue(0, getString(R.string.no).toCharArray()));
            yAxisValues.add(new AxisValue(1, getString(R.string.yes).toCharArray()));
        ...
        for (Map.Entry<String, ArrayList<HistoryItem>> entry : symptomsHistory.entrySet()) {
            ...
            for (int i = 0; i < datesAndAnswers.size(); i++) {
                ...
                if (!yAxisValues.containsKey(itemIndex)) {
                    yAxisValues.put(itemIndex,
                            new AxisValue(itemIndex, itemAnswer.substring(0, itemAnswer.length() > 8 ? 8 : itemAnswer.length()).toCharArray()));
                }
            }
